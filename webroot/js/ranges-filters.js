/**
 * Ranges - Amélioration UX des filtres et sélection en masse
 */
...
$(document).ready(function() {
    ...
    function updateSelectedCount() {
        ...
        $('#selectedCount').text(checked + ' plage(s) sélectionnée(s)');
    ...
    $('#bulkActionsForm').on('submit', function(e) {
        ...
        if (checked === 0) {
            e.preventDefault();
            alert('Aucune plage sélectionnée.');
            return false;
        }

        if (!confirm('Êtes-vous sûr de vouloir supprimer ' + checked + ' plage(s) ?')) {
            e.preventDefault();
            return false;
        }
    });
